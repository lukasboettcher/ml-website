...
export class ACGAN {
    ...
    buildDiscriminator() {
        ...
        const image = tf.input({ shape: [this.IMAGE_SIZE, this.IMAGE_SIZE, 1] });
        ...
        const aux = tf.layers.dense({ units: this.NUM_CLASSES, activation: 'softmax' })
    ...
    async trainDiscriminatorOneStep(
        ...
        const [x, y, auxY] = tf.tidy(() => {
            ...
            const sampledLabels =
                tf.randomUniform([batchSize, 1], 0, this.NUM_CLASSES, 'int32')
                    .asType('float32');
            ...
            const y = tf.tidy(
                () => tf.concat(
                    [tf.ones([batchSize, 1]).mul(this.SOFT_ONE), tf.zeros([batchSize, 1])]));
    ...
    async trainCombinedModelOneStep(batchSize, latentSize, combined) {
        ...
        const [noise, sampledLabels, trick] = tf.tidy(() => {
            ...
            // not-fake.
            const trick = tf.tidy(() => tf.ones([batchSize, 1]).mul(this.SOFT_ONE));
    ...
    async run(epochs: number, batchSize: number, latentSize: number, learningRate: number, adamBeta1: number) {
        ...
        let { xs: xTrain, labels: yTrain } = this.mnistData.getTrainData();
